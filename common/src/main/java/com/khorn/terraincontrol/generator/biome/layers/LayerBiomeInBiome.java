        ...
        for (int n = 0; n < z_size; n++)
        {
            for (int i1 = 0; i1 < x_size; i1++)
            {
                ...
                if (inOcean)
                {
                    ...
                    {
                        currentPiece = (currentPiece & RiverBits) | LandBit | biome.getIds().getGenerationId() | IslandBit;
                        spawn = true;
                    }
                }
                if (!spawn)
                {
                    ...
                    if (BiomeIsles[(currentPiece & BiomeBits)] && BiomeIsles[i2] && BiomeIsles[i3] && BiomeIsles[i4] && BiomeIsles[i5] && nextInt(chance) == 0)
                        currentPiece = (currentPiece & LandBit) | (currentPiece & RiverBits) | biome.getIds().getGenerationId() | IslandBit;
